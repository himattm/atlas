# Minimap Codex Planning Brief — corrected implementation brief

Use this document as the source brief for Codex when asking it to produce a detailed implementation plan for Minimap.
...
```text
Read `MINIMAP_CODEX_PLANNING_BRIEF.md` fully before proposing changes.

Your task is to create a very detailed implementation plan for Minimap. Do not start coding yet.

Minimap is shared navigation memory and soft validation for AI agents working in Android codebases. It captures how agents navigate a launched Android app, stores that knowledge as a repo-committed graph, and lets future agents reuse the graph to save tokens/time while detecting unexpected differences from the repo’s expected navigation state.
...
The product name is **Minimap**.

Use these names unless the implementation plan explicitly calls out a packaging conflict:

```text
Product: Minimap
Default CLI namespace: minimap
Repo directory: .minimap/
Primary skill: minimap-app-navigation
Codex planning brief: MINIMAP_CODEX_PLANNING_BRIEF.md
Implementation plan: docs/MINIMAP_IMPLEMENTATION_PLAN.md
```

The `minimap` CLI name has public prior art, including database tooling. For the planning phase, use `minimap`. Before public distribution, evaluate whether the final binary/package should be renamed to avoid collisions.
...
Minimap is not only for a special class of “Android coding agents.” The agent is generic: Codex, Claude Code, Gemini CLI, Android Studio Agent Mode, or another AI coding agent. The codebase/runtime domain is Android.

Minimap captures app navigation learned during agent runs and stores it as a repo-committed graph of screens, tap recipes, routes, checks, and expectations. Future agents and teammates reuse that graph instead of rediscovering the UI from scratch.

## One-line product statement

> **Minimap turns agent navigation into committed repo memory: faster reuse when the app behaves as expected, and soft validation when it does not.**
...
AI agents working in Android repos often need to operate the running app. Without Minimap, a typical agent loop looks like this:
...
2. **Expectation baseline**: if the current app diverges from the committed graph, Minimap calls out the difference as possible expected change, selector drift, or regression.

The key savings are not that Minimap eliminates all device observation. In verified mode Minimap may still inspect the current screen. The key savings are that Minimap returns compact match/route/check results instead of repeatedly feeding full layout JSON into the agent’s context.
...
4. **Make learning natural**
   - Agents should use Minimap instead of raw layout/tap loops.
   - Minimap should wrap Android CLI primitives so exploration automatically becomes learnable.

5. **Package as a CLI plus standard skills**
   - Minimap must be usable by Codex, Claude Code, Gemini CLI, Android Studio Agent Mode, and future agents.
...
Minimap must explicitly position itself against nearby tools so Codex does not accidentally design a duplicate testing framework.

## Maestro

Maestro is a mobile UI automation/testing framework with declarative YAML flows. Minimap is different: Minimap’s primary artifact is shared navigation memory for AI agents, not human-authored test flows. Minimap routes may validate behavior, but the graph’s primary job is to help agents reuse navigation knowledge across sessions and teammates.

## Arbigent

Arbigent is an AI-agent-driven testing framework for Android/iOS/web. Minimap is not trying to be an autonomous QA agent. Minimap is repo-committed memory that any coding agent can use while working in an Android codebase.

## agent-device

agent-device provides mobile device control, accessibility snapshots, replayable scripts, and deterministic automation across platforms. Minimap should learn from its token-efficient snapshot and replay ideas, but the Minimap wedge is repo-committed, multi-agent navigation memory plus soft validation from the committed graph.

## Android CLI

Android CLI provides official low-level Android agent primitives. Minimap should sit above it. Android CLI helps an agent observe and act in the current session. Minimap helps agents remember and share navigation knowledge across sessions, branches, agents, and teammates.
...
Minimap must use the real Android CLI surface, not invented commands.

## Android CLI commands Minimap can wrap
...
1. `android layout --diff` returns changes since the last in-session internal layout snapshot, not differences against Minimap’s committed graph.
...
## Minimap command semantics must disambiguate diff concepts
...
Minimap committed-graph divergence:
  minimap drift --json
  minimap validate --json
```

Avoid a generic `minimap diff` in v1 unless it is namespaced, for example:

```bash
minimap graph diff --base main --json
```

## Minimap tap is a composition, not a direct Android CLI wrapper

`minimap tap` must resolve and execute input through one of these paths:
...
Minimap should sit above Android CLI.
...
Android CLI gives low-level observe/describe/screen primitives. Minimap adds persistence, normalization, route lookup, check evaluation, staged graph updates, and compact agent outputs.

## Important implementation principle

Minimap commands should feel like better versions of the loops agents already use.
...
Minimap should support this:

```bash
minimap layout --json
minimap tap --selector "text=Settings" --reason "open settings" --json
...
2. Agent asks Minimap for a route.
3. Minimap does not know one.
4. Agent explores through Minimap wrappers.
5. Minimap records layout observations, actions, transitions, and checks.
6. Agent reaches target.
7. Minimap turns the run into staged graph updates.
8. Human or explicit instruction accepts the graph updates.
9. The graph is committed to the repo.
```

The first selector comes from the agent reading `minimap layout --json` output. Minimap does not magically choose every first-run action. The value is that the first-run decision becomes captured and reusable.
...
2. Agent asks Minimap for a route.
3. Minimap finds a route or resolves a path through the graph.
4. Minimap navigates using known edges and tap recipes.
5. Minimap returns compact check results instead of full layout JSON.
...
```text
1. Minimap uses the committed graph to navigate.
2. The running app does not match the expected screen/edge/check.
3. Minimap classifies the mismatch.
4. Minimap either flags possible regression or stages a graph update proposal.
...
2. Minimap runs a budgeted discovery process.
3. Minimap stages screens, edges, routes, and checks.
...
Real apps have different graphs under different conditions. Minimap must model this from v1.
...
If the current context cannot satisfy a guard, Minimap should not classify the route as broken. It should return:
...
Minimap should stage proposals automatically, but not accept them automatically.
...
Exact fingerprints are insufficient. Minimap needs a robust screen matching algorithm.
...
Do not commit the success/failure counts by default. They live in `.minimap/state/`.

---

# 12. Compose-first selector strategy

Minimap must assume many Compose apps do not expose broad stable IDs or test tags.
...
Minimap may encourage teams to add stable semantics/test tags over time, but v1 cannot require complete test tag coverage. `minimap doctor` should report selector quality and recommend improvements, not fail the entire product when tags are missing.
...
Normalized coordinates are useful but fragile. Minimap must treat tap caches as conditional accelerators, not universal truth.
...
If graph fallback finds a route different from the named preferred route, Minimap should not silently rewrite the route. It can stage a proposal if the new path is validated.
...
├── routes/
│   ├── read-article.minimap.json
│   ├── bookmark-article.minimap.json
│   └── share-article.minimap.json
├── checks/
...
Do not commit `.minimap/graph/index.json` in v1.
...
- optionally cache generated index under .minimap/state/index-cache.json, gitignored
...
.minimap/routes/*.json
.minimap/checks/*.json
skill files installed by minimap init
...
All committed Minimap JSON should use:
...
5. Raw observations remain in .minimap/runs/ and are gitignored by default.
```
...
```bash
minimap init [--dry-run] [--yes] [--agents codex,claude,android-studio,gemini] [--skill-paths ...]
```
...
.minimap/routes/
.minimap/checks/
.gitignore entries for .minimap/runs/ and .minimap/state/
repo-local skill directories selected by --agents/--skill-paths
optional AGENTS.md / CLAUDE.md / GEMINI.md guidance snippets
```

`minimap init` must be idempotent. It must not overwrite existing instructions without showing a diff or using `--yes`. It must support `--dry-run`.
...
```bash
minimap observe start <name> --json
...
`minimap layout` wraps `android layout` and adds:
...
`minimap layout --diff` wraps `android layout --diff`. This is an Android in-session layout delta, not a graph-vs-app drift report.

```bash
minimap tap --selector "<selector>" --reason "<why>" --json
minimap tap --point 0.51,0.34 --reason "<why>" --json
minimap tap --label 5 --screenshot .minimap/runs/current/ui.png --reason "<why>" --json
```
...
The implementation plan must decide whether `minimap tap` always observes after action or supports `--no-observe-after`.
...
```bash
minimap go <target> --json
minimap navigate <target> --json
```
...
```bash
minimap go article-detail --mode safe
minimap go article-detail --mode verified
minimap go article-detail --mode fast
```
...
```json
{
  "schema_version": "minimap.result.v1",
  "status": "passed",
...
```bash
minimap validate --json
minimap validate --since HEAD~1 --json
...
```bash
minimap permissions check --json
minimap permissions grant --json
```
...
```json
{
  ...
  "recommended_next_command": "minimap go article-detail --mode verified --json",
...
Minimap should not simply say “diff changed.” It should classify divergence.
...
The main adoption hurdle is getting agents to use Minimap instead of raw Android CLI layout/tap loops.

## Strategy decision

Minimap should use **multi-write repo-local skill installation** in v1.

Do not rely on the Android skills catalog for Minimap v1. Android CLI can install skills from its catalog, but Minimap should not assume it can contribute to that catalog or that all agents will use it.

`minimap init` should generate identical or adapter-specific `SKILL.md` files into the repo-local locations selected by config/flags.
...
over `minimap-android-ui` so the skill name does not permanently lock the product to Android, while its description and body can clearly say that v1 is for Android codebases.
...
`minimap init --agents auto` should detect existing agent directories and write to those. `minimap init --agents all` may write to all known repo-local paths. `minimap doctor` should report which agents are covered.
...
Use Minimap whenever you need to navigate, inspect, tap, check, validate, or learn the launched Android app UI.

## Core rule

Before using raw Android layout or adb tap commands:

1. Ask Minimap whether the repo already knows the route.
2. If Minimap knows the route, use Minimap to navigate.
3. If Minimap does not know the route, explore through Minimap wrappers so the run can be learned and shared.
...
```bash
minimap observe start <task-name> --json
minimap layout --json
```
...
```bash
minimap tap --selector "<best selector>" --reason "<why this tap is needed>" --json
minimap layout --diff --json
minimap check --current --json
```
...
Do not run `minimap accept`, `minimap update-baseline`, or commit `.minimap/` changes without explicit user approval.

## Fallback

Only use raw Android CLI layout or adb tap commands if Minimap cannot perform the needed action. If you bypass Minimap, explain why and prefer adding the observed navigation back into Minimap afterward.
```
...
```markdown
## Android UI navigation with Minimap

This repo uses Minimap for shared navigation memory and soft validation for AI agents working in this Android codebase.

When working with the launched Android app UI:

1. Use the `minimap-app-navigation` skill.
2. Run `minimap route <target> --json` before exploring manually.
3. Use `minimap go <target> --json` for known screens.
4. If no route exists, use `minimap observe`, `minimap layout`, `minimap tap`, and `minimap learn --stage`.
5. Do not call raw `android layout` or adb tap commands first.
6. Do not accept Minimap graph updates or commit `.minimap/` changes without explicit user approval.
```
...
Minimap should normalize Android layout output into stable graph facts.
...
```json
{
  ...
  "skills": {
    "skill_name": "minimap-app-navigation",
...
Every committed Minimap object must include `schema_version`.

Rules:

```text
- If Minimap reads a newer major schema version, fail closed with a clear error.
- If Minimap reads an older supported minor schema, it may auto-upgrade in memory.
- Persistent migrations require `minimap migrate` and should show a diff.
...
```bash
minimap migrate --from minimap.screen.v1 --to minimap.screen.v2 --dry-run --json
minimap migrate --apply --json
```
...
Minimap must detect common Android system overlays before classifying app routes as broken:
...
Many apps embed WebViews. Minimap v1 should not promise full WebView introspection.
...
```text
docs/MINIMAP_IMPLEMENTATION_PLAN.md
```
...
```text
.minimap/graph contains at least one screen and one edge.
.minimap/routes contains a route to article-detail.
...
```text
Minimap reaches article-detail with fewer layout calls and less layout JSON returned to the agent than the first run.
Minimap reports estimated layout calls saved and whether full layout JSON was avoided.
...
```text
Minimap detects divergence from the committed graph.
Minimap classifies the divergence.
Minimap stages a graph update if the change appears learnable.
Minimap does not silently update the baseline.
```
...
```text
Minimap returns context_mismatch rather than route_broken.
Minimap recommends establishing the required context or using a logged-out route variant.
```
...
> How often does Minimap avoid returning full layout JSON to the agent after the graph is learned?
...
1. Minimap is for AI agents working in Android codebases.
2. The shared graph is committed at the repo level.
3. The graph is both navigation cache and soft validation baseline.
4. Minimap wraps Android CLI and composes with adb for input execution; it does not replace Android CLI.
5. Minimap v1 assumes the app is already built/launched.
...
1. Implement `.minimap/config.json` loading and validation.
2. Implement `minimap init --dry-run` with idempotent file generation.
3. Implement `minimap doctor`.
4. Implement Android CLI adapter with mocked tests.
5. Implement adb executor abstraction with mocked tests.
6. Implement `minimap layout --json` and `minimap layout --diff --json`.
7. Implement observation run storage under `.minimap/runs/`.
8. Implement redaction-before-hashing.
9. Implement normalization of layout output.
10. Implement screen matching similarity.
11. Implement `minimap tap` layout-selector path.
12. Implement `minimap tap` coordinate path.
...
15. Implement `minimap learn --stage` proposal generation.
16. Implement `minimap accept`.
17. Implement `minimap route` with preferred path and graph fallback.
18. Implement `minimap go` in verified mode.
19. Implement `minimap check`.
20. Implement `minimap drift` and `minimap validate`.
21. Implement multi-write skill installation.
22. Implement `minimap repair`.
23. Implement `minimap map --discover --max-actions` only after core reuse works.
...
- treat Minimap as an Android-only agent rather than a tool for AI agents working in Android codebases
...
These notes are for planning how Minimap should integrate with Codex and other agents:
...
- Android CLI can install Android skills from its catalog, but Minimap v1 should write its own repo-local skill files directly.
...
```text
Use `MINIMAP_CODEX_PLANNING_BRIEF.md` as the source of truth. Create `docs/MINIMAP_IMPLEMENTATION_PLAN.md` as a detailed living implementation plan. Inspect the repo first. Do not implement yet. The plan must cover CLI, Android CLI/adb adapter, graph context, screen matching, route resolution, graph schemas, repo storage, skill installation, AGENTS.md guidance, staged self-healing proposals, soft validation, privacy/redaction, test strategy, milestones, and acceptance criteria. Preserve the product framing: Minimap is shared navigation memory and soft validation for AI agents working in Android codebases.
```
